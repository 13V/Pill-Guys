...
export default {
  ...
  decks: [
    ...
    { kind: 'strip', x0: 23, x1: 37, z: -3, w: 2 },                 // B1 SAFE  (far)       x:23..37  (clear continuous w2 walk @8u/s)
    // RISKY (near, z+3): a long FORWARD-CONVEYOR shortcut (pushes +X -> ~12u/s vs the
    // safe lane's 8). You ride the belt the whole branch except the one void jump-gap:
    // the size-4 gauntlet SITS ON the first belt (jump the spikes, land back on the belt —
    // the L1/L3 model), then a 4u void jump-gap, then a short belt into hub D. The belt
    // is the time-save; the gauntlet + the gap are the risk (two on-line challenges).
    { kind: 'conveyor', cx: 28.5, cz: 3, len: 11, w: 4 },           // B1 RISKY belt (+X)   x:23..34  (~12u/s; run-up + gauntlet cx29 sits on it; jump spikes, land on belt)
    { kind: 'conveyor', cx: 39,   cz: 3, len: 2,  w: 4 },           // B1 RISKY bridge end  x:38..40  (belt after a 4u void jump-gap x34..38; rides into hub D)
    ...
    { kind: 'strip', x0: 60, x1: 68, z: -3, w: 2 },                 // B2 SAFE  (far)       x:60..68  (clear continuous w2 walk @8u/s; flush off F)
