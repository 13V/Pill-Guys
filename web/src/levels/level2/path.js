...
export default {
  ...
  decks: [
    ...
    { kind: 'conveyor', cx: 5, cz: 3, len: 4, w: 4 },               // RISKY lead-in belt x: 3..7   (cz3 near lane; boosts the risky runner across hub A toward branch 1)
    ...
    { kind: 'strip', x0: 8, x1: 21, z: -3, w: 2 },                  // B1 SAFE (far)      x: 8..21  (clear, plain 8 u/s)
    { kind: 'conveyor', cx: 10,   cz: 3, len: 4, w: 4 },            // B1 RISKY run-up belt  x: 8..12  (+X boost into the gauntlet leap)
    { kind: 'conveyor', cx: 18.5, cz: 3, len: 5, w: 4 },            // B1 RISKY landing belt x:16..21  (+X boost after the gauntlet -> hub C)

    { kind: 'platform', cx: 24, cz: 0, w: 6, d: 6, rails: true },   // C saw hub          x:21..27  (rejoin 1 + decor saws; entry hub branch 2)
    { kind: 'conveyor', cx: 24, cz: 3, len: 8, w: 4 },              // RISKY hub-C boost  x:20..28  (cz3 near lane only; carries the risky runner across the rejoin hub at ~12 u/s)
    ...
    { kind: 'strip', x0: 29, x1: 37, z: -3, w: 2 },                 // D2 SAFE (far)      x:29..37  (clear continuous precision walk, plain 8 u/s)
    { kind: 'conveyor', cx: 30, cz: 3, len: 6, w: 4 },              // D2 RISKY run-up belt  x:27..33  (+X boost from hub C across the run-up into the jump-gap leap)
